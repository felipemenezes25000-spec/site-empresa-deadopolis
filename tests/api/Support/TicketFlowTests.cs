...
{
    [Fact]
    public void ResponseAndResolutionRecordSlaTimestamps()
    {
        ...
        var ticket = new Ticket(
            ...
            "Descrição suficientemente longa para teste.",
            new SlaDeadlines(now.AddHours(8), now.AddHours(40)));
        ticket.RecordResponse(now.AddMinutes(2));
        Assert.Equal("IN_PROGRESS", ticket.Status);
        Assert.NotNull(ticket.FirstResponseAt);
        ticket.Resolve(now.AddMinutes(5));
