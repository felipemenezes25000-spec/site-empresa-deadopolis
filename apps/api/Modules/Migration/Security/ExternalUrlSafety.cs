...
public static class ExternalUrlSafety
{
    public static bool IsAllowedUri(Uri uri, string allowedHost)
    {
        ...
        if (uri.Scheme is not (Uri.UriSchemeHttp or Uri.UriSchemeHttps)) return false;
        if (!string.IsNullOrEmpty(uri.UserInfo)) return false;
        if (!uri.IsDefaultPort && uri.Port is not (80 or 443)) return false;
    ...
    public static async Task<IPAddress[]> ResolvePublicAddressesAsync(string host, CancellationToken cancellationToken)
    {
        ...
        if (addresses.Length == 0) throw new InvalidOperationException("Host não resolveu para nenhum endereço IP.");
