...
public static partial class TransparencyEndpoints
{
    ...
    public static IEndpointRouteBuilder MapTransparencyEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        var admin = endpoints.MapGroup("/api/v1/admin/datasets")
            ...
            .RequireAuthorization(p => p.RequireClaim("capability", "datasets.manage"));
    ...
    private static async Task<IResult> ListPublishedAsync(ApplicationDbContext db, CancellationToken ct)
    {
        ...
        return Results.Ok(datasets.Select(x => new
        {
            ...
            x.ReferencePeriod,
            x.LastUpdatedAt,
            x.NextExpectedUpdateAt,
            x.Source,
            latestVersion = versionMap.GetValueOrDefault(x.Id)
        }));
    }
    ...
    private static async Task<IResult> GetPublishedAsync(string slug, ApplicationDbContext db, CancellationToken ct)
    {
        var dataset = await db.Datasets.AsNoTracking()
            .SingleOrDefaultAsync(x => x.Slug == slug && x.Status == DatasetStatus.Published, ct);
    ...
    {
        var dataset = await db.Datasets.AsNoTracking()
            .SingleOrDefaultAsync(x => x.Id == datasetId && x.Status == DatasetStatus.Published, ct);
        if (dataset is null) return Results.NotFound();

        var item = await db.DatasetVersions.AsNoTracking()
            .SingleOrDefaultAsync(x => x.DatasetId == datasetId && x.Version == version, ct);
        if (item is null) return Results.NotFound();
        if (storage.State == "NOT_CONFIGURED")
            return Results.Problem(title: "Storage não configurado", statusCode: StatusCodes.Status503ServiceUnavailable);
        ...
        CancellationToken ct)
    {
        ...
        if (slug is null)
            return Results.ValidationProblem(new Dictionary<string, string[]> { ["slug"] = ["Slug deve conter apenas letras minúsculas, números e hífens."] });
        if (await db.Datasets.AnyAsync(x => x.Slug == slug, ct))
            return Results.Conflict(new { title = "Slug já cadastrado", status = 409 });
        ...
        {
            return Results.ValidationProblem(new Dictionary<string, string[]> { ["dataset"] = [ex.Message] });
        }
    ...
    private static async Task<IResult> UpdateAsync(
        ...
        CancellationToken ct)
    {
        var dataset = await db.Datasets.SingleOrDefaultAsync(x => x.Id == id, ct);
        if (dataset is null) return Results.NotFound();
        try
        {
            dataset.UpdateMetadata(
        ...
        CancellationToken ct)
    {
        var dataset = await db.Datasets.SingleOrDefaultAsync(x => x.Id == id, ct);
        if (dataset is null) return Results.NotFound();
        if (!await db.DatasetVersions.AnyAsync(x => x.DatasetId == id, ct))
            return Results.Conflict(new { title = "Publique ao menos uma versão de arquivo antes do dataset." });
        ...
        AddAudit(db, tenant, principal, context, "dataset.published", dataset.Id, new { dataset.Status, dataset.PublishedAt });
        await db.SaveChangesAsync(ct);
        return Results.Ok(dataset);
    }
        ...
        CancellationToken ct)
    {
        var dataset = await db.Datasets.SingleOrDefaultAsync(x => x.Id == id, ct);
        if (dataset is null) return Results.NotFound();
        dataset.Archive(DateTimeOffset.UtcNow);
        AddAudit(db, tenant, principal, context, "dataset.archived", dataset.Id, new { dataset.Status });
        await db.SaveChangesAsync(ct);
        return Results.Ok(dataset);
    }
        ...
        string metadataJson = "{}")
    {
        ...
        if (dataset.Status == DatasetStatus.Archived)
            return Results.Conflict(new { title = "Dataset arquivado não recebe novas versões." });
        if (storage.State == "NOT_CONFIGURED")
        ...
        if (file.Length <= 0 || file.Length > MaxDatasetBytes)
            return Results.ValidationProblem(new Dictionary<string, string[]> { ["file"] = ["Arquivo deve possuir até 50 MB."] });
        ...
        {
            return Results.ValidationProblem(new Dictionary<string, string[]> { ["metadataJson"] = ["Metadados devem ser JSON válido."] });
        }
        ...
        var detected = DetectDataset(bytes, file.FileName);
        if (detected is null)
            return Results.Problem(
                title: "Formato não permitido",
                detail: "São aceitos CSV, JSON, XLSX e PDF validados pelo conteúdo/assinatura do arquivo.",
                statusCode: StatusCodes.Status415UnsupportedMediaType);

        var sha = Convert.ToHexString(SHA256.HashData(bytes)).ToLowerInvariant();
        if (await db.DatasetVersions.AnyAsync(x => x.DatasetId == id && x.Sha256 == sha, ct))
            return Results.Conflict(new { title = "Este conteúdo já foi versionado no dataset." });

        var nextVersion = (await db.DatasetVersions.Where(x => x.DatasetId == id).MaxAsync(x => (int?)x.Version, ct) ?? 0) + 1;
        var objectKey = $"datasets/{dataset.Id:N}/v{nextVersion}/{Guid.NewGuid():N}.{detected.Value.Extension}";
        ...
        var version = new DatasetVersion(
            ...
            metadataJson,
            now);
        dataset.MarkVersionPublished(now);
        ...
        AddAudit(
            ...
            version.Id,
            new { datasetId = dataset.Id, version.Version, version.FileName, version.Sha256, version.Format, storage.State });
        await db.SaveChangesAsync(ct);
    ...
    private static (string Mime, string Extension, string Format)? DetectDataset(byte[] bytes, string fileName)
    {
        ...
        if (extension == ".xlsx" && bytes.Length >= 4 && bytes[0] == 0x50 && bytes[1] == 0x4B && bytes[2] == 0x03 && bytes[3] == 0x04)
            return ("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "xlsx", "XLSX");
    ...
    public sealed record DatasetCreateRequest(
        ...
        string? Source,
        DateTimeOffset? NextExpectedUpdateAt);

    public sealed record DatasetUpdateRequest(
        string Title,
        string Description,
        string Category,
        string ResponsibleDepartment,
