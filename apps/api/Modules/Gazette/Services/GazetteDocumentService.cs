...
public sealed class GazetteDocumentService
{
    ...
    public string NormalizeComposition(GazetteComposition composition)
    {
        ...
        if (composition.Sections is null || composition.Sections.Count is 0 or > 80) throw new ArgumentException("A edição precisa possuir entre 1 e 80 seções.", nameof(composition));
        var sections = composition.Sections.Select(section =>
        {
            var title = RequireText(section.Title, 160, "Título da seção");
            if (section.Acts is null || section.Acts.Count is 0 or > 250) throw new ArgumentException($"A seção '{title}' precisa possuir entre 1 e 250 atos.", nameof(composition));
            var acts = section.Acts.Select(act => new GazetteActInput(RequireText(act.Title, 220, "Título do ato"), RequireText(act.Body, 50_000, "Conteúdo do ato"), OptionalText(act.Organization, 180), OptionalText(act.LegalReference, 300))).ToArray();
    ...
    {
        if (string.IsNullOrWhiteSpace(json)) throw new ArgumentException("A composição do Diário é obrigatória.", nameof(json));
        var composition = JsonSerializer.Deserialize<GazetteComposition>(json, JsonOptions) ?? throw new ArgumentException("A composição do Diário é inválida.", nameof(json));
    ...
    public GazetteDocumentResult Generate(GazetteEdition edition, string publicVerificationBaseUrl)
    {
        ...
        var canonicalSnapshot = JsonSerializer.Serialize(new { edition.Number, edition.Year, type = edition.Type.ToString().ToUpperInvariant(), publicationDate = edition.PublicationDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture), composition }, JsonOptions);
    ...
    private static List<string> BuildLines(GazetteEdition edition, GazetteComposition composition, string contentHash, string verificationCode)
    {
        var lines = new List<string>
        {
            "PREFEITURA MUNICIPAL DE DEODÁPOLIS - MS",
            "DIÁRIO OFICIAL ELETRÔNICO",
            FormattableString.Invariant($"Edição {edition.Number}/{edition.Year} · {edition.Type}"),
            $"Data de publicação: {edition.PublicationDate.ToString("dd/MM/yyyy", CultureInfo.InvariantCulture)}",
        ...
        foreach (var section in composition.Sections)
        {
            ...
            foreach (var act in section.Acts) lines.Add($"  - {act.Title}");
        }
        foreach (var section in composition.Sections)
        {
            lines.Add(string.Empty); lines.Add(section.Title.ToUpperInvariant()); lines.Add(new string('-', Math.Min(72, section.Title.Length + 8)));
    ...
    private static byte[] BuildPdf(IReadOnlyList<string> lines, string verificationUrl)
    {
        ...
        var pageRefs = Enumerable.Range(0, pageChunks.Length).Select(index => FormattableString.Invariant($"{4 + (index * 2)} 0 R"));
        objectBodies[2] = $"<< /Type /Pages /Kids [{string.Join(' ', pageRefs)}] /Count {pageChunks.Length.ToString(CultureInfo.InvariantCulture)} >>";
        ...
        {
            var pageObject = 4 + (index * 2); var contentObject = pageObject + 1;
            objectBodies[pageObject] = FormattableString.Invariant($"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 3 0 R >> >> /Contents {contentObject} 0 R >>");
            ...
            objectBodies[contentObject] = FormattableString.Invariant($"<< /Length {streamLength} >>\nstream\n{stream}\nendstream");
        }
        ...
        for (var objectNumber = 1; objectNumber <= objectBodies.Count; objectNumber++)
        {
            ...
            WriteLatin1(output, FormattableString.Invariant($"{objectNumber} 0 obj\n{objectBodies[objectNumber]}\nendobj\n"));
        }
        var xrefOffset = output.Position;
        WriteLatin1(output, FormattableString.Invariant($"xref\n0 {objectBodies.Count + 1}\n0000000000 65535 f \n"));
        for (var objectNumber = 1; objectNumber <= objectBodies.Count; objectNumber++) WriteLatin1(output, FormattableString.Invariant($"{offsets[objectNumber]:D10} 00000 n \n"));
        WriteLatin1(output, FormattableString.Invariant($"trailer\n<< /Size {objectBodies.Count + 1} /Root 1 0 R >>\nstartxref\n{xrefOffset}\n%%EOF\n"));
    ...
    private static string BuildPageStream(IReadOnlyList<string> lines, int page, int pageCount, string? verificationUrl)
    {
        ...
        foreach (var line in lines) builder.Append('(').Append(EscapePdf(line)).Append(") Tj T*\n");
        builder.Append("ET\n");
        builder.Append(CultureInfo.InvariantCulture, $"BT /F1 8 Tf 48 28 Td (Página {page} de {pageCount}) Tj ET\n");
        if (!string.IsNullOrWhiteSpace(verificationUrl))
        {
            ...
            builder.Append(CultureInfo.InvariantCulture, $"BT /F1 7 Tf 420 638 Td ({EscapePdf("Verifique no portal oficial")}) Tj ET\n");
        }
        return builder.ToString();
    }

    private static void AppendQr(StringBuilder builder, List<BitArray> matrix, decimal originX, decimal originY, decimal moduleSize)
    {
        ...
        for (var row = 0; row < matrix.Count; row++)
        {
            for (var column = 0; column < matrix[row].Count; column++)
            {
                ...
                var x = originX + (column * moduleSize); var y = originY - (row * moduleSize);
    ...
    private static IEnumerable<string> Wrap(string value, int width)
    {
        ...
        {
            if (current.Length > 0 && current.Length + word.Length + 1 > width) { yield return current.ToString(); current.Clear(); }
    ...
    private static void WriteLatin1(Stream stream, string value) { var bytes = Encoding.Latin1.GetBytes(value); stream.Write(bytes, 0, bytes.Length); }
    private static string RequireText(string value, int maxLength, string label) { if (string.IsNullOrWhiteSpace(value)) throw new ArgumentException($"{label} é obrigatório."); var normalized = value.Trim(); if (normalized.Length > maxLength) throw new ArgumentException($"{label} deve possuir até {maxLength} caracteres."); return normalized; }
    private static string? OptionalText(string? value, int maxLength) { if (string.IsNullOrWhiteSpace(value)) return null; var normalized = value.Trim(); if (normalized.Length > maxLength) throw new ArgumentException($"Campo opcional deve possuir até {maxLength} caracteres."); return normalized; }
}
