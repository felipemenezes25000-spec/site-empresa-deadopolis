...
public static class DatabaseInitializer
{
    ...
    private static void AddDemoUsers(ApplicationDbContext database, IPasswordHasher<UserAccount> passwordHasher, string password)
    {
        ...
        AddCapabilities(database, "SUPER_ADMIN", "audit.read", "content.write", "content.review", "content.publish", "resources.manage", "services.manage", "gazette.write", "gazette.sign", "gazette.publish", "media.manage", "mail.manage", "migration.manage", "support.write", "users.manage", "settings.manage");
        AddCapabilities(database, "COMMUNICATION", "content.write", "content.review", "content.publish", "resources.manage", "media.manage");
    ...
    private static PortalResource[] DemoResources()
    {
        var resources = new[]
        {
            ...
            new PortalResource(DemoMunicipalityId, "DATASET", "catalogo-demonstracao", "Catálogo de dados — demonstração", "Exemplo de dataset estruturado para a POC.", "{\"format\":[\"CSV\",\"JSON\"],\"updateFrequency\":\"Mensal\",\"license\":\"Dados abertos - demonstração\",\"files\":[],\"classification\":\"DEMONSTRATION\"}", 1, DemoActorId),
