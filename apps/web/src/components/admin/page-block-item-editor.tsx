...
const mediaBlockTypes = new Set<PageBlockType>(["Gallery", "QuickAccess", "FeaturedNews", "NewsGrid", "ServiceGrid", "DepartmentGrid", "CustomLinks"]);
const dateBlockTypes = new Set<PageBlockType>(["Events", "Documents", "FeaturedNews", "NewsGrid"]);
...
export function PageBlockItemEditor({ blockType, item, index, images, disabled, onChange, onRemove }: { blockType: PageBlockType; item: PageBlockItem; index: number; images: ApprovedImage[]; disabled: boolean; onChange: (patch: Partial<PageBlockItem>) => void; onRemove: () => void }) {
  ...
  const supportsMedia = mediaBlockTypes.has(blockType);
  const supportsDate = dateBlockTypes.has(blockType);
  ...
  return <fieldset className="grid gap-3 rounded-lg border border-border bg-surface p-3"><legend className="px-1 font-semibold">Item {number}</legend>
    {supportsMedia && <label className="field">{gallery ? `Imagem aprovada ${number}` : `Imagem do item ${number} (opcional)`}<select value={images.some((image) => `/api/v1/media/${image.id}` === item.mediaUrl) ? item.mediaUrl : ""} disabled={disabled} onChange={(event) => { if (!event.target.value) { onChange({ mediaUrl: "", mediaAlt: "" }); return; } const image = images.find((entry) => `/api/v1/media/${entry.id}` === event.target.value); if (image) onChange({ mediaUrl: event.target.value, mediaAlt: image.altText || image.originalFileName, label: item.label || image.originalFileName }); }}><option value="">{gallery ? "Escolha na biblioteca" : "Sem imagem"}</option>{images.map((image) => <option key={image.id} value={`/api/v1/media/${image.id}`}>{image.originalFileName} — {image.altText || "sem ALT"}</option>)}</select></label>}
    ...
    {supportsDate && <label className="field">Data do item {number}<input type="date" value={item.date} disabled={disabled} onChange={(event) => onChange({ date: event.target.value })} /></label>}
    {supportsMedia && <><label className="field">URL da mídia interna {number}<input value={item.mediaUrl} pattern="/api/v1/media/.*" maxLength={2048} disabled={disabled} onChange={(event) => onChange({ mediaUrl: event.target.value })} /></label><label className="field">Texto alternativo da imagem {number}<input value={item.mediaAlt} maxLength={500} disabled={disabled} onChange={(event) => onChange({ mediaAlt: event.target.value })} /><small>Obrigatório sempre que houver imagem; sem ALT a publicação é recusada.</small></label></>}
