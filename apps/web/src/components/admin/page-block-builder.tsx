...
export function PageBlockBuilder({ initialBlocks, disabled = false }: { initialBlocks: PageBlock[]; disabled?: boolean }) {
  ...
  return <section className="grid gap-3" aria-labelledby="page-builder-title">
    ...
    <div className="grid gap-3">{blocks.map((block, index) => <article key={block.id} className="rounded-xl border border-border bg-surface-soft p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><strong>Bloco {index + 1}</strong><small className="ml-2 text-muted">{block.type}</small></div><div className="button-row"><button type="button" className="action-button secondary" disabled={disabled || index === 0} onClick={() => move(index, -1)} aria-label={`Mover bloco ${index + 1} para cima`}>↑</button><button type="button" className="action-button secondary" disabled={disabled || index === blocks.length - 1} onClick={() => move(index, 1)} aria-label={`Mover bloco ${index + 1} para baixo`}>↓</button><button type="button" className="action-button secondary" disabled={disabled} aria-label={`Remover bloco ${index + 1}`} onClick={() => setBlocks((current) => current.filter((item) => item.id !== block.id))}>Remover</button></div></div>
      <div className="editor-fields">
        <label className="field">Tipo do bloco {index + 1}<select value={block.type} disabled={disabled} onChange={(event) => updateBlock(block.id, { type: event.target.value as PageBlockType })}>{pageBlockTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
        <label className="field">Título do bloco {index + 1}<input value={block.title} maxLength={220} disabled={disabled} onChange={(event) => updateBlock(block.id, { title: event.target.value })} /></label>
        <label className="field">Conteúdo do bloco {index + 1}<textarea rows={4} value={block.content} maxLength={4000} disabled={disabled} onChange={(event) => updateBlock(block.id, { content: event.target.value })} /></label>
