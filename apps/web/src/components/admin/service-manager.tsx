...
export function ServiceManager() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/v1/admin/services", { signal: controller.signal })
      .then(async (response) => {
        if (response.ok && !controller.signal.aborted) setItems(await response.json() as Service[]);
      })
      .catch(() => undefined);
  ...
  async function load() {
    ...
    if (response.ok) setItems(await response.json() as Service[]);
  }
  ...
  return <div className="editor-grid">
    <section className="admin-panel"><h2>Catálogo atual</h2><div className="compact-list">{items.map((item) => <div className="compact-item" key={item.id}><div><strong>{item.name}</strong><small style={{ display: "block" }}>{item.area} · /{item.slug}</small></div><span className="status-pill">{item.status}</span></div>)}</div></section>
