...
export function SearchAutocomplete({ defaultValue = "" }: { defaultValue?: string }) {
  ...
  const isLoading = query.length >= 2 && loading;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      void fetch(`/api/v1/search/suggest?q=${encodeURIComponent(normalizedQuery)}`, { signal: controller.signal })
        .then(async (response) => {
  ...
  return <div className="content-search">
    ...
    />
    <datalist id={listId}>{visibleSuggestions.map((item) => <option key={`${item.type}-${item.url}`} value={item.title}>{item.type}</option>)}</datalist>
    <button type="submit">Buscar</button>
    <span id={`${listId}-hint`} className="sr-only" aria-live="polite">{isLoading ? "Buscando sugestões" : `${visibleSuggestions.length} sugestões disponíveis`}</span>
  </div>;
}
